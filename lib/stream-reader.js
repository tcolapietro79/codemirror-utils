...
export default class CodeMirrorStreamReader extends StreamReader {
	...
	constructor(editor, pos, limit) {
		...
		this._eof = limit
			? limit.to
			: CodeMirror.Pos(lastLine, this._lineLength(lastLine));
	...
	backUp(n) {
		...
		this.pos =
			line < 0 || ch < 0 ? CodeMirror.Pos(0, 0) : CodeMirror.Pos(line, ch);
	...
	error(message) {
		const err = new Error(
			`${message} at line ${this.pos.line}, column ${this.pos.ch}`
		);
